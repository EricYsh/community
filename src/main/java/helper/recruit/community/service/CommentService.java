...
import helper.recruit.community.model.*;
...
public class CommentService {
    ...
    public List<CommentDTO> listByQuestionId(Long id) {
        ...
        // lambda get commentators
        Set<Long> commentators = comments.stream().map(comment -> comment.getCommentator()).collect(Collectors.toSet());
        List<Long> usersIds = new ArrayList<>();
        usersIds.addAll(commentators); // change to user id

        // get commentator and transfer into map
        UserExample userExample = new UserExample();
        userExample.createCriteria().andIdIn(usersIds);
        List<User> users = userMapper.selectByExample(userExample);
        Map<Long, User> userMap = users.stream().collect(Collectors.toMap(user -> user.getId(), user -> user));

        // transfer comment to commentDTO
